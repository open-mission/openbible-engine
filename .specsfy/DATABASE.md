...
## Persistência remota Sync

- O adapter `@openbible/adapter-sync-turso` usa Turso/libSQL via Kysely e mantém
  um schema versionado separado do namespace Better Auth. A migration atual é a
  versão 1 e usa `CREATE TABLE IF NOT EXISTS` dentro de transação para as
  estruturas do Sync.
- As tabelas remotas são `sync_devices`, `sync_notes`, `sync_revisions`,
  `sync_tombstones`, `sync_operations`, `sync_bible_preferences` e
  `sync_account_deletion_jobs`, além de `sync_schema_migrations`. Todas as
  estruturas de negócio carregam `account_id`; operações têm cursor técnico
  crescente e chave única `(account_id, operation_id)`.
- `sync_notes` e `sync_revisions` armazenam somente o envelope serializado; o
  conteúdo da nota, título, referência e datas da nota não entram no schema.
  `sync_tombstones` retém exclusões por 90 dias. Jobs de exclusão de conta são
  idempotentes por conta e têm prazo de 30 dias.
- Fonte executável: `packages/adapter-sync-turso/src/schema.ts` e
  `packages/adapter-sync-turso/src/adapter.ts`. Nenhum valor de conexão ou
  token é documentado.
...
| Dispositivo autorizado | Permitir que a pessoa reconheça, aprove, revogue e acompanhe os dispositivos usados na sincronização | Apelido do dispositivo, data e horário de autorização, data e horário da última sincronização e situação ativa ou revogada | Apelido como texto curto; autorização e última sincronização como data e horário; situação como escolha ativa ou revogada | Cada dispositivo pertence a uma conta autenticada e participa do ciclo de aprovação, sincronização e revogação das notas dessa conta | A pessoa dona da conta consulta e revoga; o Sync consulta a situação para autorizar ou bloquear novas sincronizações | É criado quando a pessoa aprova o dispositivo; a última sincronização é atualizada durante o uso; a situação pode mudar para revogada; a revogação bloqueia novas sincronizações, mas não apaga automaticamente cópias locais; o registro remoto sai quando a conta é excluída conforme a retenção definida | specs/backlog/0008-sincronizacao-multidispositivo-offline-first-com-turso.md;specs/completed/0007-bounded-context-personal-study-offline/spec.md |
| Histórico de versões da nota | Permitir resolver conflitos entre dispositivos e manter o registro mínimo das decisões de sincronização | Conteúdo protegido de cada versão, dispositivo de origem, data e horário da alteração, situação do conflito e ligação com a revisão final escolhida ou mesclada | Uma entrada por versão; conteúdo como texto protegido; dispositivo como identificação opaca; data e horário do evento; situação como conflito ou resolvido; ligação para a revisão final | Cada versão permanece ligada a uma nota, a um dispositivo autorizado e, quando resolvida, à nova revisão resultante | O Sync mantém e compara o estado técnico; a pessoa dona da conta consulta as versões por meio do consumidor e escolhe ou mescla a revisão final | É criado quando há alterações concorrentes ou colisão na importação; permanece enquanto o conflito estiver aberto; após resolução fica retido por 30 dias; sai junto com os dados remotos quando a conta é excluída conforme o prazo de até 30 dias | specs/backlog/0008-sincronizacao-multidispositivo-offline-first-com-turso.md;specs/completed/0007-bounded-context-personal-study-offline/spec.md |
| Alteração pendente de sincronização | Retomar alterações locais quando houver rede ou autenticação e explicar o estado da sincronização sem expor conteúdo pessoal | Nota afetada, ação de criar/alterar/excluir, data e horário, situação, número de tentativas, código do último erro e ordem de processamento | Uma entrada por alteração; nota identificada de forma opaca; ação como escolha criar, alterar ou excluir; data e horário; situação; número; código curto de erro; ordem | Cada alteração permanece ligada a uma nota, ao dispositivo que a produziu e à conta autenticada quando for sincronizável | O Sync consulta e altera a situação, tentativas e erro; a pessoa consulta o estado por meio do consumidor; o conteúdo da nota não entra no acompanhamento técnico | É criada quando uma mutação local sincronizável ocorre; muda durante tentativas, bloqueio por autenticação ou conflito; o payload sai após confirmação remota, permanecendo somente o estado mínimo para idempotência e retomada; falha local de armazenamento não cria a entrada | specs/backlog/0008-sincronizacao-multidispositivo-offline-first-com-turso.md;specs/completed/0007-bounded-context-personal-study-offline/spec.md |
| Versão bíblica sincronizada | Permitir que outro dispositivo reconheça a versão que a pessoa já escolheu e faça o redownload oficial para uso offline | Identificação e nome da versão, versão do conteúdo, data e horário da instalação, situação disponível ou pendente e referência da origem oficial | Uma entrada por versão; identificação e nome como textos curtos; versão do conteúdo como texto; instalação como data e horário; situação como escolha disponível ou pendente; origem como referência para redownload | Cada versão fica ligada à conta autenticada e à preferência de instalação; não guarda nem transporta o arquivo bíblico pelo Sync | A pessoa consulta por meio do consumidor; o Sync atualiza a situação; o consumidor usa a origem oficial para redownload | É criada quando a pessoa instala ou escolhe uma versão; a situação muda para disponível ou pendente conforme o redownload; uma indisponibilidade não remove a escolha; o registro remoto sai quando a conta é excluída conforme a retenção definida | specs/backlog/0008-sincronizacao-multidispositivo-offline-first-com-turso.md;specs/completed/0001-openbible-engine-scripture-library/spec.md;docs/database.md |
