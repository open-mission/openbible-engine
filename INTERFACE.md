...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Não há blocos React | — | — | — | CLI `apps/conformance-cli/src/index.ts` com comandos `check`, `list-books`, `get-chapter`, `search`, `parse` que consomem apenas exports públicos `createBibleEngine` | success JSON, empty array, error `EngineError` code | Desenvolvedores | Não aplicável |

## Telas e composição

| Tela ou rota | Arquivo | Componentes React usados | Dados e ações | Estados |
| --- | --- | --- | --- | --- |
| Não há tela de produto | `apps/conformance-cli/src/index.ts` | Nenhum (CLI Node) | Comandos via `process.argv`, instanciação de fakes/adapters, saída JSON, exit 0/1 | success, empty, error |
