...
export class NativeBibleLibrary implements BibleLibrary {
  private store = new Map<string, NativeStoreEntry>();
  ...
  // Engine will call one of these after validation
  ...
  async installPackage(versionId: string, bytes: Uint8Array): Promise<void> {
    // Validate header & parse to populate store
    const parsed = parseSyntheticBytes(bytes);
    // Optionally also load into driver for completeness
    if (typeof (this.driver as InMemoryDriver).load === "function") {
      try {
        (this.driver as InMemoryDriver).load(bytes, versionId);
      } catch {
        // ignore
      }
    }
  ...
  // Uninstall hooks for engine
  async uninstall(versionId: string): Promise<void> {
    this.store.delete(versionId);
    const d = this.driver as InMemoryDriver;
    if (typeof d.delete === "function") d.delete(versionId);
  }
