...
var EngineError = class _EngineError extends Error {
  ...
  constructor(code, message, options) {
    ...
    if (options?.cause !== void 0 && this.cause === void 0) {
      this.cause = options.cause;
    }
    Object.setPrototypeOf(this, _EngineError.prototype);
  }
};

// ../engine-core/dist/book-meta.js
var BOOKS = [
  { id: "gen", name: "G\xEAnesis", abbreviation: "Gn", testament: "old", chapters: 50 },
  { id: "exo", name: "\xCAxodo", abbreviation: "Ex", testament: "old", chapters: 40 },
  { id: "lev", name: "Lev\xEDtico", abbreviation: "Lv", testament: "old", chapters: 27 },
  { id: "num", name: "N\xFAmeros", abbreviation: "Nm", testament: "old", chapters: 36 },
  { id: "deu", name: "Deuteron\xF4mio", abbreviation: "Dt", testament: "old", chapters: 34 },
  { id: "jos", name: "Josu\xE9", abbreviation: "Js", testament: "old", chapters: 24 },
  { id: "jdg", name: "Ju\xEDzes", abbreviation: "Jz", testament: "old", chapters: 21 },
  ...
  { id: "1ch", name: "1 Cr\xF4nicas", abbreviation: "1Cr", testament: "old", chapters: 29 },
  { id: "2ch", name: "2 Cr\xF4nicas", abbreviation: "2Cr", testament: "old", chapters: 36 },
  ...
  { id: "job", name: "J\xF3", abbreviation: "J\xF3", testament: "old", chapters: 42 },
  { id: "psa", name: "Salmos", abbreviation: "Sl", testament: "old", chapters: 150 },
  { id: "pro", name: "Prov\xE9rbios", abbreviation: "Pv", testament: "old", chapters: 31 },
  { id: "ecc", name: "Eclesiastes", abbreviation: "Ec", testament: "old", chapters: 12 },
  { id: "sng", name: "C\xE2nticos", abbreviation: "Ct", testament: "old", chapters: 8 },
  { id: "isa", name: "Isa\xEDas", abbreviation: "Is", testament: "old", chapters: 66 },
  { id: "jer", name: "Jeremias", abbreviation: "Jr", testament: "old", chapters: 52 },
  { id: "lam", name: "Lamenta\xE7\xF5es", abbreviation: "Lm", testament: "old", chapters: 5 },
  ...
  { id: "amo", name: "Am\xF3s", abbreviation: "Am", testament: "old", chapters: 9 },
  ...
  { id: "jhn", name: "Jo\xE3o", abbreviation: "Jo", testament: "new", chapters: 21 },
  { id: "act", name: "Atos", abbreviation: "At", testament: "new", chapters: 28 },
  { id: "rom", name: "Romanos", abbreviation: "Rm", testament: "new", chapters: 16 },
  { id: "1co", name: "1 Cor\xEDntios", abbreviation: "1Co", testament: "new", chapters: 16 },
  { id: "2co", name: "2 Cor\xEDntios", abbreviation: "2Co", testament: "new", chapters: 13 },
  { id: "gal", name: "G\xE1latas", abbreviation: "Gl", testament: "new", chapters: 6 },
  { id: "eph", name: "Ef\xE9sios", abbreviation: "Ef", testament: "new", chapters: 6 },
  ...
  { id: "1ti", name: "1 Tim\xF3teo", abbreviation: "1Tm", testament: "new", chapters: 6 },
  { id: "2ti", name: "2 Tim\xF3teo", abbreviation: "2Tm", testament: "new", chapters: 4 },
  ...
  { id: "1jo", name: "1 Jo\xE3o", abbreviation: "1Jo", testament: "new", chapters: 5 },
  { id: "2jo", name: "2 Jo\xE3o", abbreviation: "2Jo", testament: "new", chapters: 1 },
  { id: "3jo", name: "3 Jo\xE3o", abbreviation: "3Jo", testament: "new", chapters: 1 },
...
var BOOK_BY_ABBR = Object.fromEntries(BOOKS.map((b) => [b.abbreviation.toLowerCase(), b]));
...
// src/worker/sqlite.ts
var loadSqlite3 = sqlite3_bundler_friendly_default;
async function createOpfsPool(options = {}) {
...
function buildPoolLike(pool, _sqlite3) {
  return {
...
// src/worker/paths.ts
var REGISTRY_DB = "/store.db";
function finalPath(versionId) {
  return `/${versionId}.db`;
}
function backupPath(versionId) {
  return `/${versionId}.db.bak`;
}
function trashPath(versionId) {
  return `/${versionId}.db.trash`;
}
function temporaryPath(versionId, nonce) {
  return `/${versionId}.db.tmp-${nonce}`;
}

// src/worker/registry.ts
function toBible(row) {
  return {
    id: String(row.id),
...
var WebRegistry = class {
  ...
  db;
  async list() {
    const rows = this.db.prepare("SELECT id, name, installed_at, version_code FROM installed_bibles ORDER BY id").all();
    return rows.map(toBible);
  }
  async get(id) {
    const row = this.db.prepare("SELECT id, name, installed_at, version_code FROM installed_bibles WHERE id = ?").get(id);
  ...
  listSync() {
    return this.db.prepare("SELECT id, name, installed_at, version_code FROM installed_bibles ORDER BY id").all().map(toBible);
  }
  getSync(id) {
    const row = this.db.prepare("SELECT id, name, installed_at, version_code FROM installed_bibles WHERE id = ?").get(id);
    return row ? toBible(row) : null;
...
// src/worker/legacy-book-map.ts
var INT_TO_BOOK = /* @__PURE__ */ new Map();
var BOOK_TO_INT = /* @__PURE__ */ new Map();
BOOKS.forEach((book, index) => {
  const intId = index + 1;
  INT_TO_BOOK.set(intId, book);
  BOOK_TO_INT.set(book.id, intId);
});
function bookIdToInt(bookId) {
  return BOOK_TO_INT.get(bookId);
}
function intToBook(intId) {
  return INT_TO_BOOK.get(intId);
}
function intToCanonicalId(intId) {
  return INT_TO_BOOK.get(intId)?.id;
}
var MAX_BOOK_ID = BOOKS.length;

// src/worker/library.ts
function rowToVerse(raw) {
...
var WebLibrary = class {
  ...
  async getChapter(versionId, bookId, chapter) {
    ...
    if (intId === void 0) return [];
    const rows = this.require(versionId).prepare("SELECT book_id, chapter, verse, text FROM verse WHERE book_id = ? AND chapter = ? ORDER BY verse").all(intId, chapter);
    const verses = [];
    for (const r of rows) {
      const v2 = rowToVerse(r);
      if (v2) verses.push(v2);
    }
    return verses;
  }
  async search(versionId, query, limit) {
    const db = this.require(versionId);
    if (!query || query.trim().length === 0) return { versionId, query, results: [], total: 0 };
    const pattern = `%${query}%`;
    const total = Number(
      db.prepare("SELECT COUNT(*) AS c FROM verse WHERE text LIKE ? COLLATE NOCASE").get(pattern).c
    );
    const rows = db.prepare(
  ...
  async getVersionName(versionId) {
    return readMetadata(this.require(versionId), "name");
  }
};

// src/worker/installer.ts
var SQLITE_HEADER = new TextEncoder().encode("SQLite format 3\0");
...
function validateImportedPackage(db, expectedVersionId) {
  ...
  const row = db.prepare("SELECT value FROM metadata WHERE key = 'versionId'").get();
  const idRow = db.prepare("SELECT value FROM metadata WHERE key = 'id'").get();
  ...
  const bookCount = Number(db.prepare("SELECT COUNT(*) AS c FROM book").get().c);
  const verseCount = Number(db.prepare("SELECT COUNT(*) AS c FROM verse").get().c);
  ...
  const nameRow = db.prepare("SELECT value FROM metadata WHERE key = 'name'").get();
  return { name: nameRow?.value ?? expectedVersionId, versionId: storedVersionId ?? void 0 };
}
var WebInstaller = class {
  constructor(pool, registry) {
    this.pool = pool;
    this.registry = registry;
  }
  async install(input, observer) {
  ...
  async compensate(final, bak, tmp, hadPrevious, promoted) {
    if (this.hasFile(bak)) {
  ...
  async uninstall(versionId) {
    const final = finalPath(versionId);
    const trashPath2 = `${final}.trash`;
    ...
    if (hasStorage) {
      ...
      await this.pool.importDb(trashPath2, bytes);
      await this.pool.unlink(final);
    }
    try {
      if (hasRegistry) await this.registry.remove(versionId);
    } catch (e) {
      if (hasStorage && this.hasFile(trashPath2)) {
        const bytes = await this.pool.exportFile(trashPath2);
        await this.pool.importDb(final, bytes);
        await this.pool.unlink(trashPath2);
      }
      throw new EngineError("storage_unavailable", "Failed to remove installed bible", { cause: e });
    }
    if (hasStorage && this.hasFile(trashPath2)) await this.pool.unlink(trashPath2);
  }
...
async function reconcilePool(pool, registry) {
  ...
  const present = (p) => pool.fileNames().includes(p);
  for (const id of ids) {
...
var WorkerRuntime = class {
  ...
  async install(operationId, input, requestId) {
    const cancellable = { aborted: Boolean(input.token?.aborted), reason: input.token?.reason };
    this.cancellables.set(operationId, cancellable);
    const progress = (input2) => {
      this.emitProgress(requestId, operationId, input2);
    };
  ...
  emitProgress(requestId, operationId, progress) {
...
async function requestPersistentStorage(requested) {
  if (requested === false) return "unsupported";
...
function isBusyError(error) {
  ...
  return message.includes("busy") || message.includes("already") || message.includes("in use") || message.includes("pool") || message.includes("locked") || message.includes("another open access") || message.includes("access handle");
}
function codeFromWorkerError(error) {
